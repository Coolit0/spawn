...
//Can Add	
	...
	if ($_SESSION['user']['can_add'] or $_SESSION['user']['admin']){
		$add_button="<a href='contacts.ncr.admin.php'><span class='glyphicon glyphicon-pencil'> Add New Contact </span></a>";
		$can_add=1;
		} else {
			$add_button=null;
			$can_add=0;
...
try {
	...
	if ($keyword <> ""){
    $sql = "SELECT * FROM tbl_contacts WHERE region = 'NCR' AND " . " (last_name LIKE :keyword) ORDER BY last_name ";
    $stmt = $DB->prepare($sql);
    ...
    $sql = "SELECT * FROM tbl_contacts WHERE region = 'NCR'ORDER BY last_name ";
    $stmt = $DB->prepare($sql);
  }
...
<div class="col-lg-12" style="padding-left: 0; padding-right: 0;" >
	  <div class="pull-left"><a href="index_ncr.admin.php"><button class="btn btn-primary"><span class="glyphicon glyphicon-home"></span> Home</button></a>
		<a href="index_users.php"><button class="btn btn-primary"><span class="glyphicon glyphicon-user"></span> Users</button></a>
	  <?php if ($can_add) { 
		echo $add_button;} ?>	
	  </div>
	  
	<form action="index_ncr.admin.php" method="get" >
...
<?php foreach ($results as $res) { ?>
                ...
                <?php $pic = ($res["profile_pic"] <> "" ) ? $res["profile_pic"] : "no_avatar.png" ?>
				  ...
				  <?php if ($can_edit){ ?>
				  <td><a href="contacts_ncr.admin.php?m=update&cid=<?php echo $res["contact_id"]; ?>&pagenum=<?php echo $_GET["pagenum"]; ?>"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;</td>
				  <?php } ?>
  ...
  for ($i = 1; $i <= $last; $i++) {
    if ($i == $pagenum) {
      ?>
                ...
                <?php } else { ?>
                <li><a href="index_ncr.admin.php?pagenum=<?php echo $i; ?>&keyword=<?php echo $_GET["keyword"]; ?>" class="links"  onclick="displayRecords('<?php echo $page_limit; ?>', '<?php echo $i; ?>');" ><?php echo $i ?></a></li>
                <?php }      } ?>
