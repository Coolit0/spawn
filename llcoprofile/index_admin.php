...
//Can Add	
	...
	if ($_SESSION['user']['can_add'] or $_SESSION['user']['admin']){
		$add_button="<a href='contacts.php'><span class='glyphicon glyphicon-pencil'> Add New Contact </span></a>";
		$can_add=1;
		} else {
			$add_button=null;
			$can_add=0;
...
try {
	...
	if ($keyword <> ""){
    $sql = "SELECT * FROM tbl_contacts WHERE 1 AND " . " (last_name LIKE :keyword) ORDER BY last_name ";
    $stmt = $DB->prepare($sql);
    ...
    $sql = "SELECT * FROM tbl_contacts WHERE 1 ORDER BY last_name ";
    $stmt = $DB->prepare($sql);
  }
...
<br>
  <div class="panel panel-primary">
    <div class="panel-heading">
      <h3 class="panel-title">Labor Inspectors</h3>
    </div>
		...
		<br>
          <table class="table table-striped table-hover table-bordered ">
			...
			<tr>
				...
				<th>Status</th>
				<th>Position</th>
				<th>Course for Labor Inspectors</th>
				<th>Last Perf. Rating</th>
				<th>Issued w/ Cert. of Good Standing</th>
				<?php if ($can_edit) {
...
<?php foreach ($results as $res) { ?>
                ...
                <?php $pic = ($res["profile_pic"] <> "" ) ? $res["profile_pic"] : "no_avatar.png" ?>
				  ...
				  <?php if ($can_edit){ ?>
				  <td><a href="contacts.php?m=update&cid=<?php echo $res["contact_id"]; ?>&pagenum=<?php echo $_GET["pagenum"]; ?>"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;</td>
				  <?php } ?>
				  <?php if ($can_delete){ ?>
				  <td><a href="process_form.php?mode=delete&cid=<?php echo $res["contact_id"]; ?>&keyword=<?php echo $_GET["keyword"]; ?>&pagenum=<?php echo $_GET["pagenum"]; ?>" onclick="return confirm('Are you sure?')"><span class="glyphicon glyphicon-trash"></span></a>&nbsp;</td>
				  <?php } ?>
  ...
  for ($i = 1; $i <= $last; $i++) {
    if ($i == $pagenum) {
      ?>
                ...
                <?php } else { ?>
                <li><a href="index_admin.php?pagenum=<?php echo $i; ?>&keyword=<?php echo $_GET["keyword"]; ?>" class="links"  onclick="displayRecords('<?php echo $page_limit; ?>', '<?php echo $i; ?>');" ><?php echo $i ?></a></li>
                <?php }      } ?>
