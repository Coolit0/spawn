...
if ($mode == "add_new" ) {
  ...
  if (!$error) {
    ...
    $sql = "INSERT INTO `tbl_contacts` (`first_name`, `middle_name`, `last_name`, `sex`, `contact_no1`, `contact_no2`, `email_address`, `status`, `position`, `course`, `rating`, `cert`, `region`, `office_assgn`, `profession`, `duties`, `profile_pic`, `birthday`, `username`, `password`) VALUES "
            . "( :fname, :mname, :lname, :sex, :contact1, :contact2, :email, :status, :position, :course, :rating, :cert, :region, :office_assgn, :profession, :duties, :pic, :birthday, :username, :password)";
  ...
  if (!$error) { 
    ...
    $sql = "UPDATE `tbl_contacts` SET `first_name` = :fname, `middle_name` = :mname, `last_name` = :lname, `sex` = :sex, `contact_no1` = :contact1, `contact_no2` = :contact2, `email_address` = :email, `status` = :status, `position` = :position, `course` = :course, `rating` = :rating, `cert` = :cert, `region` = :region, `office_assgn` = :office_assgn, `profession` = :profession, `duties` = :duties, `profile_pic` = :pic, `birthday` = :birthday, `username` = :username, `password` = :password "
  ...
  header("location:index_admin.php?pagenum=".$_POST['pagenum']);
} elseif ( $mode == "delete" ) {
   $cid = intval($_GET['cid']);
   ...
   header("location:index_admin.php?pagenum=".$_GET['pagenum']);
}
?>